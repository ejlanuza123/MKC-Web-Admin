...
Welcome to the Admin Web User Manual for the **MKC Foods Corporation Platform**. This guide provides step-by-step instructions for managing food & commercial orders, inventory, fleet riders, customer feedback, and real-time operational alerts effectively.
...
1. Open the MKC Admin Web Portal.
...
- **Live Sound Chime**: Whenever a customer places a new food or product order, the web app plays an ascending dual-tone chime bell (`587 Hz ➔ 880 Hz`).
- **Floating Banner**: A notification banner appears in the top-right corner displaying:
  - Order Number & Branch Name (`MKC Foods Corporation`)
...
6. **Customer Feedback Feed**: Latest customer reviews and ratings for food products and riders.
...
3. Enter Product Name, Category, Price (`₱`), Stock Quantity, Unit (`kg`, `pack`, `pc`), and upload a product image.
4. Click **Save**.
