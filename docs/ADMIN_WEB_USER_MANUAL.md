# Admin Web User Manual — MKC Foods Corporation Platform

Welcome to the Admin Web User Manual for the **MKC Foods Corporation Platform**. This guide provides comprehensive, step-by-step instructions for managing food & commercial orders, kitchen operations, store pause status, auto-dispatch, fleet riders, customers, and real-time operational alerts.
...
- **Audio Permission**: Click anywhere on the webpage once after logging in to enable Web Audio API sound alerts for incoming food orders.
...
1. Navigate to the MKC Admin Web Portal URL.
...
- **Synthesizer Chime**: When a customer places a food order, the system plays an ascending dual-tone chime (`587 Hz ➔ 880 Hz`).
- **Floating Banner Alert ([`OrderAlertBanner`](file:///c:/Projects/MKC%20Foods%20Corporation/mkc-admin-web/src/components/OrderAlertBanner.jsx))**:
  - Appears immediately in the top-right corner with order number (e.g. `#24`), total amount (`₱X.XX`), and customer delivery address.
...
- **Notification Dropdown ([`NotificationContext`](file:///c:/Projects/MKC%20Foods%20Corporation/mkc-admin-web/src/context/NotificationContext.jsx))**:
...
Manage store open/closed states and customer announcements with instant real-time sync to all customer mobile apps:

1. Open **Settings** in the sidebar.
2. Scroll to the **Store Status & Holiday / Emergency Mode** panel.
3. **Select Operational Mode**:
   - 🟢 **Normal Operations (Open)**: Store is fully active; customers can place instant food delivery orders.
   - 🔴 **Emergency Pause**: Temporarily halt food deliveries during severe weather (typhoon, heavy rain) or kitchen emergencies.
   - 🟡 **Holiday Mode**: Announce special holiday schedules, reduced kitchen hours, or festive advisories.
   - 🔵 **Scheduled Maintenance**: Announce system upgrades or scheduled branch downtime.
4. **Preset Announcements & Custom Messages**:
   - Choose a quick preset (e.g. *Severe Tropical Storm Advisory*, *National Holiday Schedule*, *Scheduled Kitchen Deep Cleaning*) or enter a custom **Announcement Title** and **Customer Notice Details**.
...
6. **Allow Scheduled Pre-Orders**:
   - If enabled, customers can still browse the food catalog and submit scheduled pre-orders for when the branch reopens.
...
Automate order assignments to eliminate kitchen dispatch bottlenecks:
...
3. **Configure Dispatch Rules**:
   ...
   - **Search Radius (km)**: Maximum distance around the branch hub to search for available riders (default: 10 km).
...
3. Click any order row to view:
   ...
   - Ordered food items, quantities, and price breakdown.
   - Kitchen preparation notes and customer delivery instructions.
...
2. Generates an official formatted PDF invoice (`Order-#24.pdf`) with complete line items, tax breakdown, and company details.
...
4. **Bulk Status Updates & Export**:
   - **`Mark Processing (N pending)`**: Moves all selected pending orders into the kitchen fulfillment pipeline.
...
2. **3-Tier Fleet Duty Status**:
   - 🟡 **On Delivery**: Rider is currently en route on an active food assignment.
...
## 8. Products & Food Inventory Management

1. Open **Products** in sidebar.
2. **Add / Edit Products**:
   - Enter Food Item Name, Category (`Frozen Goods`, `Fresh Cuts`, `Processed Foods`, `Beverages`).
   - Set Price (`₱`), Stock Quantity, Unit (`kg`, `pack`, `pc`), and upload high-res product photos.
...
1. Open **Sales Heatmap**:
   - Visualizes food delivery order density across Puerto Princesa barangays.
   - Identify peak dining hours and high-volume delivery zones.
2. Open **Reports**:
   - Select date range (`Today`, `This Week`, `This Month`, `Custom Range`).
   - Filter by payment method (`Cash on Delivery`, `GCash`).
   - **Export to Excel (`.xlsx`)**: 1-click download of full sales data for accounting and financial reporting.
...
2. View 1 to 5-star ratings ⭐ and customer comments for food items and delivery riders.
