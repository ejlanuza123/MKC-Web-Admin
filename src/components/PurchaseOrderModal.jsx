...
export default function PurchaseOrderModal({ isOpen, onClose, products = [], storeName = "MKC FOODS CORPORATION" }) {
  const [supplierName, setSupplierName] = useState('MKC Central Food & Beverage Logistics');
  ...
  const poSummary = useMemo(() => {
    ...
    const items = lowStockItems.map(p => {
      ...
      const unitCost = (p.current_price || p.price || 0) * 0.85; // Estimated wholesale cost (85% of retail)
  ...
  const handlePrintPO = () => {
    ...
    const htmlContent = `
      ...
      <body>
        ...
        <div class="footer">
          Generated via MKC Foods Corporation Platform Inventory Auto-Replenishment Module
        </div>
