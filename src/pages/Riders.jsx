      ...
      <div className={`rounded-xl max-w-2xl w-full max-h-[90vh] overflow-hidden shadow-2xl transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 text-slate-100' : 'bg-white text-gray-900'}`}>
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-180px)]">
          <div className="space-y-6">
            ...
            <div className={`rounded-xl p-4 border ${isDarkMode ? 'bg-slate-800/60 border-slate-700' : 'bg-blue-50/60 border-blue-100'}`}>
              <h5 className={`font-semibold mb-3 flex items-center gap-2 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
                <TrendingUp size={16} className="text-mkc-blue" />
                Earnings &amp; Performance
              </h5>
              <div className="grid grid-cols-3 gap-3 mb-4">
                ...
                <div className={`rounded-lg p-3 text-center ${isDarkMode ? 'bg-slate-700' : 'bg-white'}`}>
                  ...
                  <p className={`text-xs mb-0.5 ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Completion Rate</p>
                  <p className="font-bold text-mkc-blue text-sm">{perfStats.completionRate}%</p>
                </div>
                <div className={`rounded-lg p-3 text-center ${isDarkMode ? 'bg-slate-700' : 'bg-white'}`}>
                  <Clock size={16} className="text-orange-500 mx-auto mb-1" />
                  <p className={`text-xs mb-0.5 ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Avg. Time</p>
              ...
              {perfStats.weeklyData && perfStats.weeklyData.length > 0 && (
                <div>
                  <p className={`text-xs mb-2 ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Last 7 Days</p>
                    ...
                    {(() => {
                      ...
                      return perfStats.weeklyData.map((d, i) => (
                        <div key={i} className="flex-1 flex flex-col items-center gap-0.5" title={`${d.label}: ${d.count} deliveries`}>
                          <div
                            className="w-full rounded-t transition-all duration-500 bg-mkc-blue/70 hover:bg-mkc-blue"
                            style={{ height: `${Math.max((d.count / maxCount) * 52, d.count > 0 ? 4 : 0)}px` }}
                          />
                          <span className={`text-[9px] ${isDarkMode ? 'text-slate-500' : 'text-gray-400'}`}>{d.day}</span>
                        </div>
                      ));
                    })()}
                  </div>
...
const PerformanceDashboard = React.memo(({ isDarkMode }) => {
  ...
  const SortIcon = ({ col }) => {
    ...
    return sortDir === 'desc'
      ? <ChevronDown size={12} className="inline ml-0.5 text-mkc-blue" />
      : <ChevronUp size={12} className="inline ml-0.5 text-mkc-blue" />;
  };
    ...
    return (
      <div className="space-y-4">
        <div className="grid grid-cols-4 gap-4">
          {[1,2,3,4].map(i => <div key={i} className={`h-24 rounded-xl animate-pulse ${isDarkMode ? 'bg-slate-800' : 'bg-gray-100'}`} />)}
        </div>
  ...
  if (perfError) return <ErrorAlert message={perfError} onDismiss={() => setPerfError(null)} />;

  return (
    <div className="space-y-6">
      <div className={`flex gap-2 p-1 rounded-xl w-fit ${isDarkMode ? 'bg-slate-800' : 'bg-gray-100'}`}>
        {DATE_RANGES.map(r => (
          <button
            key={r.key}
            onClick={() => setDateRangeKey(r.key)}
            className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all duration-200 ${
              dateRangeKey === r.key ? 'bg-mkc-blue text-white shadow-md' : (isDarkMode ? 'text-slate-400 hover:text-white' : 'text-gray-600 hover:text-gray-900')
            }`}
          >
            {r.label}
          </button>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Total Deliveries', value: platformStats.totalDeliveries, icon: Package, color: 'text-mkc-blue' },
          ...
          <div key={label} className={`p-4 rounded-xl border ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200'}`}>
            <div className="flex items-center gap-2 mb-1">
              <Icon size={16} className={color} />
              <p className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>{label}</p>
            </div>
      ...
      <div className={`rounded-xl border overflow-hidden ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200'}`}>
        <div className={`flex items-center gap-2 px-4 py-3 border-b ${isDarkMode ? 'border-slate-700' : 'border-gray-100'}`}>
          <Award size={18} className="text-mkc-blue" />
          <h3 className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Rider Leaderboard</h3>
          <span className={`ml-auto text-xs ${isDarkMode ? 'text-slate-500' : 'text-gray-400'}`}>{sorted.length} riders</span>
        </div>
        {sorted.length === 0 ? (
          <div className="p-10 text-center">
            <Truck size={36} className={`mx-auto mb-3 ${isDarkMode ? 'text-slate-600' : 'text-gray-300'}`} />
            <p className={`text-sm ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>No delivery data for this period</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className={`border-b ${isDarkMode ? 'border-slate-700 bg-slate-800/50' : 'border-gray-100 bg-gray-50'}`}>
                  <th className={`text-left px-4 py-2.5 font-medium ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Rank</th>
                    ...
                    <React.Fragment key={rider.id}>
                      <tr
                        className={`border-b cursor-pointer transition-colors duration-150 ${
                          isDarkMode
                            ? `border-slate-700 ${isExpanded ? 'bg-slate-800/60' : 'hover:bg-slate-800/40'}`
                      ...
                      >
                        <td className="px-4 py-3">
                          {RANK_EMOJI[rank]
                            ? <span className="text-base">{RANK_EMOJI[rank]}</span>
                            : <span className={`font-bold text-sm ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>#{rank}</span>
                          }
                        </td>
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            {rider.avatar_url
                              ...
                              : <div className="w-8 h-8 bg-mkc-blue rounded-lg flex items-center justify-center text-white font-bold text-xs">{rider.full_name?.charAt(0)?.toUpperCase() || '?'}</div>
                            }
                            <div>
                              <p className={`font-medium leading-tight ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{rider.full_name}</p>
                              <p className={`text-xs ${isDarkMode ? 'text-slate-500' : 'text-gray-400'}`}>{rider.vehicle_type || 'Rider'}</p>
                            </div>
                        ...
                        <td className="px-3 py-3 text-center">
                          <div className="flex flex-col items-center gap-1">
                            <span className={`font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>{stats.total}</span>
                            <div className={`h-1 w-12 rounded-full ${isDarkMode ? 'bg-slate-700' : 'bg-gray-200'}`}>
                              <div className="h-1 rounded-full bg-mkc-blue" style={{ width: `${Math.min((stats.total / maxBar) * 100, 100)}%` }} />
                            </div>
                          </div>
                        </td>
                        <td className="px-3 py-3 text-center">
                        ...
                        <td className="px-3 py-3 text-center">
                          <div className="flex items-center justify-center gap-1">
                            <div className={`h-1.5 rounded-full ${isDarkMode ? 'bg-slate-700' : 'bg-gray-200'}`} style={{ width: '40px' }}>
                              <div className="h-1.5 rounded-full bg-gradient-to-r from-mkc-blue to-blue-400" style={{ width: `${score}%` }} />
                            </div>
                      ...
                      {isExpanded && (
                        <tr className={`border-b ${isDarkMode ? 'border-slate-700 bg-slate-800/30' : 'border-gray-100 bg-blue-50/40'}`}>
                          <td colSpan={7} className="px-6 py-4">
                              ...
                              {(() => {
                                ...
                                return weeklyData.map((d, i) => (
                                  <div key={i} className="flex-1 flex flex-col items-center gap-0.5" title={`${d.label}: ${d.count} deliveries · ₱${d.earnings.toFixed(2)}`}>
                                    <div
                                      className="w-full rounded-t bg-mkc-blue/60 hover:bg-mkc-blue transition-all duration-300"
                            ...
                            <div className="flex gap-6 mt-3">
                              <span className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>✅ Completed: <strong className={isDarkMode ? 'text-white' : 'text-gray-900'}>{stats.completed}</strong></span>
                              <span className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>❌ Failed: <strong className="text-red-500">{stats.failed}</strong></span>
                              <span className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>⏳ Pending: <strong className="text-yellow-500">{stats.pending}</strong></span>
                            </div>
                          </td>
                        </tr>
                      )}
    ...
    <div className="space-y-6">
      <div className="flex flex-wrap justify-between items-center gap-3">
        <div>
          ...
          <div className={`flex gap-1 mt-2 p-1 rounded-lg w-fit ${isDarkMode ? 'bg-slate-800' : 'bg-gray-100'}`}>
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1 rounded-md text-sm font-medium transition-all duration-200 ${
                activeTab === 'riders' ? 'bg-mkc-blue text-white shadow-sm' : (isDarkMode ? 'text-slate-400 hover:text-white' : 'text-gray-600 hover:text-gray-900')
              }`}
            >
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-3 py-1 rounded-md text-sm font-medium transition-all duration-200 ${
                activeTab === 'performance' ? 'bg-mkc-blue text-white shadow-sm' : (isDarkMode ? 'text-slate-400 hover:text-white' : 'text-gray-600 hover:text-gray-900')
              }`}
            >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {activeTab === 'riders' && (
            <button
              ...
              className="bg-mkc-blue text-white px-4 py-2 rounded-lg flex items-center gap-2 hover:opacity-90 transition-opacity text-sm font-medium"
            >
              <Plus size={18} />
              Add Rider
            </button>
