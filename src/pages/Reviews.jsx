  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className={`p-4 rounded-xl border transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200'}`}>
          <div className="flex items-center gap-2 mb-1">
            <Star size={16} className="text-amber-400 fill-amber-400" />
            <p className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Avg Product Rating</p>
          </div>
        ...
        <div className={`p-4 rounded-xl border transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200'}`}>
          <div className="flex items-center gap-2 mb-1">
            <Star size={16} className="text-amber-400 fill-amber-400" />
            <p className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Avg Rider Rating</p>
          </div>
          <p className={`text-2xl font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>
        ...
        <div className={`p-4 rounded-xl border transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200'}`}>
          <div className="flex items-center gap-2 mb-1">
            <Package size={16} className="text-mkc-blue" />
            <p className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Product Reviews</p>
          </div>
          <p className="text-2xl font-bold text-mkc-blue">{summary.totalProductReviews}</p>
        </div>

        <div className={`p-4 rounded-xl border transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200'}`}>
          <div className="flex items-center gap-2 mb-1">
            <Truck size={16} className="text-emerald-500" />
            <p className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Rider Ratings</p>
          </div>
      ...
      {loading ? (
        ...
        productReviews.length === 0 ? (
          <div className={`rounded-xl border p-12 text-center transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200'}`}>
            <MessageSquare size={48} className={`mx-auto mb-4 ${isDarkMode ? 'text-slate-600' : 'text-gray-300'}`} />
            ...
            {productReviews.map((review) => (
              <div
                ...
                className={`p-5 rounded-xl border transition-all duration-200 ${
                  isDarkMode ? 'bg-slate-900 border-slate-700 hover:border-slate-600' : 'bg-white border-gray-200 hover:border-gray-300 shadow-sm'
                }`}
              >
                <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                  <div className="flex items-center gap-3">
                    {review.products?.image_url ? (
                      ...
                      <div className="w-12 h-12 bg-blue-100 dark:bg-slate-800 rounded-lg flex items-center justify-center text-mkc-blue">
                        <Package size={24} />
                      </div>
                    )}
                    <div>
                ...
                {review.comment ? (
                  <p className={`text-sm p-3 rounded-lg ${isDarkMode ? 'bg-slate-800 text-slate-200' : 'bg-gray-50 text-gray-700'}`}>
                    "{review.comment}"
                  </p>
                ) : (
                  <p className={`text-xs italic ${isDarkMode ? 'text-slate-500' : 'text-gray-400'}`}>No written comment provided.</p>
                )}
              </div>
            ))}
          </div>
        ...
        <div className={`rounded-xl border p-12 text-center transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200'}`}>
          <Truck size={48} className={`mx-auto mb-4 ${isDarkMode ? 'text-slate-600' : 'text-gray-300'}`} />
              ...
              <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                <div className="flex items-center gap-3">
                  {item.rider?.avatar_url ? (
                    ...
                    <div className="w-12 h-12 bg-emerald-100 dark:bg-slate-800 rounded-lg flex items-center justify-center text-emerald-600">
                      <Truck size={24} />
                    </div>
                  )}
                  <div>
