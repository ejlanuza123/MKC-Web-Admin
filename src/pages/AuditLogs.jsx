...
export default function AuditLogs() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Advanced Filter Bar */}
      <div className={`rounded-xl border p-5 shadow-sm transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200'}`}>
        <div className="flex items-center gap-2 mb-4 text-mkc-blue font-semibold text-sm">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-6 gap-4 items-end">
          {/* Entity Type Filter */}
          <div>
            <label className={`block text-xs font-semibold mb-1 transition-colors duration-300 ${isDarkMode ? 'text-slate-300' : 'text-gray-700'}`}>
              Entity Category
            </label>
            <select
              value={entityType}
              onChange={(e) => setEntityType(e.target.value)}
              className={`w-full rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-mkc-blue outline-none transition-colors duration-300 ${isDarkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-white border border-gray-300'}`}
            >
        ...
        <div className="mt-4 pt-3 border-t flex flex-wrap justify-between items-center gap-2 border-gray-200 dark:border-slate-700">
          <div className="flex items-center gap-2">
            <span className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Presets:</span>
            <button
              type="button"
              onClick={() => applyDatePreset('today')}
              className={`px-2.5 py-1 text-xs border rounded-md font-medium transition ${isDarkMode ? 'border-slate-700 text-slate-300 hover:bg-slate-800' : 'border-gray-300 text-gray-700 hover:bg-gray-50'}`}
            >
              Today
            </button>
            <button
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-lg text-xs font-semibold flex items-center gap-1 transition ${isDarkMode ? 'bg-slate-800 text-slate-300 hover:bg-slate-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
          >
