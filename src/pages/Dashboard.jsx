...
const RevenueTrendChart = ({ data, isDarkMode }) => {
  ...
  return (
    <div className={`p-6 rounded-xl border transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200 shadow-sm'}`}>
      <div className="flex flex-wrap justify-between items-center gap-3 mb-4">
        <div>
          <h3 className={`font-bold text-lg ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Performance Trend</h3>
          <p className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Revenue &amp; order volume trajectory over the selected period</p>
        </div>
        <div className={`flex gap-1 p-1 rounded-lg ${isDarkMode ? 'bg-slate-800' : 'bg-gray-100'}`}>
          <button
            onClick={() => setActiveMetric('revenue')}
            className={`px-3 py-1 text-xs font-semibold rounded-md transition-all ${
              activeMetric === 'revenue' ? 'bg-mkc-blue text-white shadow-sm' : isDarkMode ? 'text-slate-400 hover:text-white' : 'text-gray-600 hover:text-gray-900'
            }`}
          >
            Revenue (₱)
          </button>
          <button
            ...
            className={`px-3 py-1 text-xs font-semibold rounded-md transition-all ${
              activeMetric === 'orders' ? 'bg-mkc-blue text-white shadow-sm' : isDarkMode ? 'text-slate-400 hover:text-white' : 'text-gray-600 hover:text-gray-900'
            }`}
          >
            Orders
          </button>
      ...
      <div className="w-full overflow-x-auto">
        <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-44 overflow-visible">
          <defs>
            <linearGradient id="mkcChartGrad" x1="0" y1="0" x2="0" y2="1">
          ...
          {/* Area fill */}
          <path d={areaD} fill="url(#mkcChartGrad)" />
          ...
          {points.map((p, i) => (
            <g key={i} className="group cursor-pointer">
              <circle cx={p.x} cy={p.y} r="4" className="fill-[#0033A0] stroke-white dark:stroke-slate-900 group-hover:r-6 transition-all" />
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    const subscription = supabase
      .channel('mkc-dashboard-realtime')
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap justify-between items-center gap-3">
        ...
        <div className="flex items-center gap-3">
            ...
            {DATE_RANGES.map((r) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all duration-150 ${
                  dateRange === r.key
                    ? 'bg-mkc-blue text-white shadow-md'
          ...
          <button
            ...
            className={`p-2 rounded-xl border transition-colors ${isDarkMode ? 'border-slate-700 text-slate-300 hover:bg-slate-800' : 'border-gray-200 text-gray-700 hover:bg-gray-50'}`}
      ...
      <div className={`p-4 rounded-xl border flex flex-wrap items-center justify-between gap-3 ${isDarkMode ? 'bg-slate-800/60 border-slate-700' : 'bg-blue-50/60 border-blue-100'}`}>
        <div className="flex items-center gap-2">
          <Zap size={18} className="text-mkc-blue dark:text-blue-400" />
          <span className={`text-sm font-semibold ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Quick Shortcuts</span>
        </div>
        <div className="flex flex-wrap gap-2">
          <Link to="/products" className="px-3 py-1.5 bg-mkc-blue text-white rounded-lg text-xs font-medium hover:opacity-90 transition flex items-center gap-1.5">
          ...
          <Link to="/orders" className={`px-3 py-1.5 border rounded-lg text-xs font-medium transition flex items-center gap-1.5 ${isDarkMode ? 'border-slate-700 text-slate-200 hover:bg-slate-800' : 'border-gray-300 text-gray-700 hover:bg-white'}`}>
            <ShoppingBag size={14} /> View Orders
          </Link>
          <Link to="/reviews" className={`px-3 py-1.5 border rounded-lg text-xs font-medium transition flex items-center gap-1.5 ${isDarkMode ? 'border-slate-700 text-amber-400 hover:bg-slate-800' : 'border-gray-300 text-amber-600 hover:bg-white'}`}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {/* Card 1: Revenue */}
        <div className={`p-4 rounded-xl border transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200 shadow-sm'}`}>
          <div className="flex justify-between items-start mb-2">
            <span className={`text-xs font-medium ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Period Revenue</span>
            <div className="p-2 rounded-lg bg-blue-100 dark:bg-slate-800 text-mkc-blue dark:text-blue-400">
              <DollarSign size={16} />
            </div>
          </div>
          <p className={`text-xl font-bold ${isDarkMode ? 'text-blue-400' : 'text-mkc-blue'}`}>{formatCurrency(stats.periodRevenue)}</p>
          <p className={`text-[11px] mt-1 ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Total: {formatCurrency(stats.totalRevenue)}</p>
        </div>

        {/* Card 2: Orders */}
        <div className={`p-4 rounded-xl border transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200 shadow-sm'}`}>
          <div className="flex justify-between items-start mb-2">
            <span className={`text-xs font-medium ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Completed Orders</span>
            <div className="p-2 rounded-lg bg-green-100 dark:bg-slate-800 text-green-600">
              <CheckCircle size={16} />
            </div>
          </div>
          <p className="text-xl font-bold text-green-600">{stats.completedOrders}</p>
          <p className={`text-[11px] mt-1 ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>{stats.pendingOrders} pending/active</p>
        </div>
        ...
        <div className={`p-4 rounded-xl border transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200 shadow-sm'}`}>
          <div className="flex justify-between items-start mb-2">
            <span className={`text-xs font-medium ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Avg Delivery</span>
            <div className="p-2 rounded-lg bg-orange-100 dark:bg-slate-800 text-orange-500">
              <Clock size={16} />
            </div>
          </div>
          <p className="text-xl font-bold text-orange-500">{stats.avgDeliveryTime}m</p>
          <p className={`text-[11px] mt-1 ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Fleet average</p>
        </div>

        {/* Card 5: Customer Rating */}
        <div className={`p-4 rounded-xl border transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200 shadow-sm'}`}>
          <div className="flex justify-between items-start mb-2">
            <span className={`text-xs font-medium ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Satisfaction</span>
            <div className="p-2 rounded-lg bg-amber-100 dark:bg-slate-800 text-amber-500">
              <Star size={16} className="fill-amber-400" />
            </div>
          </div>
          <p className="text-xl font-bold text-amber-500">{stats.avgCustomerRating} ★</p>
          <p className={`text-[11px] mt-1 ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Customer reviews</p>
        </div>

        {/* Card 6: Auto-Dispatch */}
        <div className={`p-4 rounded-xl border transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200 shadow-sm'}`}>
          <div className="flex justify-between items-start mb-2">
            <span className={`text-xs font-medium ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>Auto-Dispatch</span>
            <div className="p-2 rounded-lg bg-purple-100 dark:bg-slate-800 text-purple-600">
              <Zap size={16} />
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className={`p-6 rounded-xl border transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200 shadow-sm'}`}>
          <div className="flex justify-between items-center mb-4">
            ...
            <Link to="/riders" className="text-xs text-mkc-blue dark:text-blue-400 hover:underline font-semibold flex items-center gap-1">
          ...
          {topRiders.length === 0 ? (
            ...
            <div className="space-y-3">
              {topRiders.map(({ rider, stats, rank }) => (
                <div key={rider.id} className={`flex items-center justify-between p-3 rounded-lg ${isDarkMode ? 'bg-slate-800' : 'bg-gray-50'}`}>
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className={`text-xs ${isDarkMode ? 'text-slate-400' : 'text-gray-500'}`}>{stats.completed} deliveries completed</p>
                    </div>
                  </div>
        ...
        <div className={`p-6 rounded-xl border transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200 shadow-sm'}`}>
          <div className="flex justify-between items-center mb-4">
            <div className="flex items-center gap-2">
              <MessageSquare size={18} className="text-mkc-blue dark:text-blue-400" />
              <h3 className={`font-bold text-base ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Customer Feedback</h3>
            </div>
            <Link to="/reviews" className="text-xs text-mkc-blue dark:text-blue-400 hover:underline font-semibold flex items-center gap-1">
              View All <ArrowRight size={12} />
            </Link>
          </div>
          ...
          {recentReviews.length === 0 ? (
            ...
            <div className="space-y-3">
              {recentReviews.map((rev) => (
                <div key={rev.id} className={`p-3 rounded-lg ${isDarkMode ? 'bg-slate-800' : 'bg-gray-50'}`}>
                  ...
                  <p className={`text-xs line-clamp-2 ${isDarkMode ? 'text-slate-300' : 'text-gray-600'}`}>
      ...
      <div className={`p-6 rounded-xl border transition-colors duration-300 ${isDarkMode ? 'bg-slate-900 border-slate-700' : 'bg-white border-gray-200 shadow-sm'}`}>
        <div className="flex justify-between items-center mb-4">
          <div className="flex items-center gap-2">
            <ShoppingBag size={18} className="text-mkc-blue dark:text-blue-400" />
            <h3 className={`font-bold text-base ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>Recent Orders</h3>
          </div>
          <Link to="/orders" className="text-xs text-mkc-blue dark:text-blue-400 hover:underline font-semibold flex items-center gap-1">
        ...
        {recentOrders.length === 0 ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className={`border-b ${isDarkMode ? 'border-slate-700 text-slate-400' : 'border-gray-100 text-gray-500'}`}>
              ...
              <tbody>
                {recentOrders.map((order) => (
                  <tr key={order.id} className={`border-b ${isDarkMode ? 'border-slate-700/50 hover:bg-slate-800' : 'border-gray-50 hover:bg-gray-50'}`}>
                    <td className="py-3 px-3 font-semibold text-mkc-blue dark:text-blue-400">#{String(order.id || '').slice(0, 8)}</td>
                    <td className={`py-3 px-3 ${isDarkMode ? 'text-slate-200' : 'text-gray-800'}`}>{order.profiles?.full_name || order.customer_name || 'Customer'}</td>
