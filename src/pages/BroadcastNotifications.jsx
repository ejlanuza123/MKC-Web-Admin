          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className={`p-5 rounded-2xl border transition-colors ${isDarkMode ? 'bg-slate-800/80 border-slate-700/80' : 'bg-white border-slate-200 shadow-sm'}`}>
              <div className="flex items-center justify-between mb-3">
                <label className="text-xs font-bold uppercase tracking-wider text-amber-500">1. Select Target Audience</label>
                  ...
                  return (
                    ...
                    >
                      <div className="flex items-center justify-between mb-1.5">
                        <Icon className={`w-5 h-5 ${isSelected ? 'text-amber-500' : isDarkMode ? 'text-slate-400' : 'text-slate-500'}`} />
                        <span className={`text-xs font-bold px-1.5 py-0.5 rounded-md ${
                          isSelected ? 'bg-amber-500 text-white' : isDarkMode ? 'bg-slate-700 text-slate-300' : 'bg-slate-200 text-slate-700'
                        }`}>
                          {aud.count}
                        </span>
                      </div>
                      <p className={`font-semibold text-sm ${isSelected ? 'text-amber-500' : ''}`}>{aud.label}</p>
            ...
            <div className={`p-5 rounded-2xl border transition-colors ${isDarkMode ? 'bg-slate-800/80 border-slate-700/80' : 'bg-white border-slate-200 shadow-sm'}`}>
              <label className="text-xs font-bold uppercase tracking-wider text-amber-500 mb-3 block">2. Category & Alert Style</label>
                  ...
                  return (
                    <button
                      ...
                      className={`p-3 rounded-xl border text-center transition-all ${
                        isSelected
                          ? 'border-amber-500 bg-amber-500/10 ring-2 ring-amber-500/20 font-semibold'
                          : isDarkMode
            ...
            <form onSubmit={handleInitiateSend} className={`p-5 rounded-2xl border transition-colors ${isDarkMode ? 'bg-slate-800/80 border-slate-700/80' : 'bg-white border-slate-200 shadow-sm'}`}>
              <label className="text-xs font-bold uppercase tracking-wider text-amber-500 mb-3 block">3. Compose Message</label>
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  placeholder="e.g. 🍗 Friday Special Promo!"
                  maxLength={70}
                  required
                  className={`w-full px-3.5 py-2.5 rounded-xl border text-sm font-medium transition ${
                    isDarkMode 
                      ? 'bg-slate-900 border-slate-700 text-white placeholder-slate-500 focus:border-amber-500 focus:ring-1 focus:ring-amber-500' 
                      : 'bg-white border-slate-300 text-slate-900 placeholder-slate-400 focus:border-amber-600 focus:ring-1 focus:ring-amber-600'
                  }`}
                />
              </div>
              ...
              <div className="mb-3">
                ...
                <textarea
                  ...
                  placeholder="e.g. Enjoy 15% off all MKC Platters and Bundles from 5:00 PM to 8:00 PM today! Order now via the mobile app."
                  rows={3}
                  maxLength={250}
                  required
            ...
            <div className={`p-5 rounded-2xl border transition-colors sticky top-6 ${isDarkMode ? 'bg-slate-800/80 border-slate-700/80' : 'bg-white border-slate-200 shadow-sm'}`}>
              ...
              <div className={`mx-auto max-w-[320px] rounded-[36px] p-3 border-4 shadow-2xl transition-all ${
                ...
                <div className="text-center text-white/90 my-6">
                  <p className="text-3xl font-light tracking-tight">12:30</p>
                ...
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-1.5">
                      <div className="w-4 h-4 rounded-md bg-amber-500 flex items-center justify-center text-[9px] font-bold text-white">
                        M
                      </div>
                      <span className="text-[11px] font-bold tracking-wide uppercase text-white/90">
                        {targetAudience === 'riders' ? 'Rider Alert' : 'MKC Foods'}
                      </span>
                    </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            ...
            >
              ...
              <p className={`text-sm mb-4 leading-relaxed ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>
                ...
                <strong className="text-amber-500">{estimatedRecipientCount} active account(s)</strong>. 
              ...
              <div className={`p-3 rounded-xl border mb-6 text-xs space-y-1 ${
                ...
                <p className="font-bold text-sm text-amber-500">{title}</p>
              ...
              <div className="flex items-center justify-end gap-2.5">
                ...
                <button
                  ...
                  disabled={isSending}
                  className="px-5 py-2 rounded-xl text-xs font-semibold bg-amber-500 hover:bg-amber-600 text-white flex items-center gap-2 shadow-md shadow-amber-500/30"
                >
                  {isSending ? (
                    <>
