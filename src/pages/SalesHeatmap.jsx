...
// Known San Pedro & Laguna key barangays & zones with centroid coordinates
...
<body>
  ...
  <script>
    ...
    L.marker([14.3583, 121.0583], { icon: storeIcon, zIndexOffset: 2000 })
      .addTo(map)
      .bindPopup('<div class="custom-popup" style="font-size:13px;line-height:1.4"><b>📍 MKC Foods Central Kitchen</b><br/><span style="color:#64748b">Hub & Logistics Center</span></div>');
...
export default function SalesHeatmap() {
  ...
  const fetchHeatmapData = useCallback(async () => {
    try {
      ...
      console.error('Error fetching MKC heatmap data:', err);
  ...
  const { heatmapPoints, markerPoints, barangayStats, summaryKpis } = useMemo(() => {
    ...
    filteredOrders.forEach(o => {
      ...
      (o.order_items || []).forEach(it => {
        const cat = it.products?.category || 'Menu Item';
  ...
  const handleExport = async (format) => {
    ...
    try {
      if (format === 'csv') {
        ...
        saveAs(blob, `mkc-sales-density-heatmap-${dateRange}-${new Date().toISOString().split('T')[0]}.csv`);
      } else if (format === 'excel') {
        const workbook = new ExcelJS.Workbook();
        const sheet = workbook.addWorksheet('MKC Sales Heatmap');

        sheet.addRow(['MKC FOODS CORPORATION - GEOGRAPHIC SALES DENSITY REPORT']);
        ...
        saveAs(blob, `mkc-sales-density-heatmap-${dateRange}-${new Date().toISOString().split('T')[0]}.xlsx`);
      }
    } catch (err) {
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className={`text-sm mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>
            Thermographic visualizer of food delivery order density, high-revenue corridors, and barangay market penetration.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          ...
          <div>
            ...
            <select
              ...
              className={`w-full px-3 py-2 text-xs rounded-lg border outline-none font-medium ${isDarkMode ? 'bg-slate-700 border-slate-600 text-white' : 'bg-gray-50 border-gray-300 text-gray-900'}`}
            >
              <option value="all">All Menu Categories ({categories.length})</option>
              {categories.map(cat => (
      ...
      <div className={`rounded-xl border overflow-hidden transition-colors ${isDarkMode ? 'bg-slate-800/90 border-slate-700' : 'bg-white border-gray-200 shadow-sm'}`}>
        ...
        <div className="h-[480px] w-full relative">
          <iframe
            ref={iframeRef}
            srcDoc={mapHtml}
            title="MKC Sales Density Heatmap"
