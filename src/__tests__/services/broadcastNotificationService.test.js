...
describe('broadcastNotificationService (MKC)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    global.fetch = vi.fn();
  ...
  describe('PRESET_TEMPLATES and BROADCAST_CATEGORIES', () => {
    ...
    it('contains preset templates for weather and promos', () => {
      ...
      const promoPreset = PRESET_TEMPLATES.find(p => p.id === 'friday_promo');
      expect(promoPreset).toBeDefined();
      expect(promoPreset.title).toContain('Friday Special Promo');
    });
  });
